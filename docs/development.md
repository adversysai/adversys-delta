# Development manual for Delta
This guide will show you how to setup a local development environment for Delta in a VS Code compatible IDE, including proper debugger.
...
> [!NOTE]
> - Delta runs in a Docker container, this simplifies installation and ensures unified environment and behavior across systems.
...
- Delta is available on GitHub [github.com/Deltaai/agent-zero](https://github.com/Deltaai/agent-zero).
- You can download the files using a browser and extract or run `git clone https://github.com/Deltaai/agent-zero` in your desired directory.

> [!NOTE]
> In my case, I used `cd ~/Desktop` and `git clone https://github.com/Deltaai/agent-zero`, so my project folder is `~/Desktop/agent-zero`.
...
- Delta comes with `.vscode` folder that contains basic setup, recommended extensions, and debugger profiles. These will help us a lot.
...
2. Prepare the python environment to run Delta in. (⚠️ This step assumes you have some Python runtime installed.) By clicking the python version in lower right corner (3.13.1 in my example), you should get a list of available environments. You can click the `+ Create Virtual Environment` button. You might be prompted to select the environment manager if you have multiple installed. I have venv and Conda, I will select Conda here. I'm also prompted for desired python version, I will select 3.12, that is known to work well.
...
## Step 4: Run Delta in the IDE
Great work! Now you should be able to run Delta from your IDE including real-time debugging.
...
After inserting my API key in settings, my Delta instance works. I can send a simple message and get a response.
...
## Step 5: Run another instance of Delta in Docker
...
1. Pull the docker image `Deltaai/agent-zero` from Docker Hub and run it with a web port (`80`) mapped and SSH port (`22`) mapped.
...
You have successfully set up a complete Delta development environment! You now have:
...
- ✅ The ability to develop, debug, and test Delta features seamlessly

You're now ready to contribute to Delta, create custom extensions, or modify the framework to suit your needs. Happy coding! 💻✨
