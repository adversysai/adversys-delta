...
class FileBrowser:
    ...
    def get_files(self, current_path: str = "") -> Dict:
        try:
            ...
            # Check if directory exists, create it if it doesn't (for knowledge directories)
            if not full_path.exists() or not full_path.is_dir():
                # Try to create the directory if it's a knowledge path
                if "knowledge" in str(full_path):
                    try:
                        full_path.mkdir(parents=True, exist_ok=True)
                        PrintStyle.success(f"Created knowledge directory: {full_path}")
                    except (OSError, PermissionError) as e:
                        PrintStyle.error(f"Directory does not exist and could not be created: {full_path} - {e}")
                        return {"entries": [], "current_path": current_path, "parent_path": ""}
                else:
                    PrintStyle.error(f"Directory does not exist: {full_path}")
                    return {"entries": [], "current_path": current_path, "parent_path": ""}
